...
typedef struct {
	void (*on_append)(uint64_t commit, const char* data, int data_len, void* cb_data);
	void (*on_commit)(uint64_t commit, void* cb_data);
...
void
ab_confirm_append(ab_node_t* node, uint64_t commit);
